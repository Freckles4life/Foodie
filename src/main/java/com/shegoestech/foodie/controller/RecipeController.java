package com.shegoestech.foodie.controller;

import com.shegoestech.foodie.models.Recipe;
import com.shegoestech.foodie.service.RecipeService;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;

import javax.validation.Valid;

@RequiredArgsConstructor
@Controller
@RequestMapping("/recipe")
public class RecipeController {
    private final RecipeService recipeService;

    @GetMapping
    public String index(Model model) {
        model.addAttribute("recipes", recipeService.getAll());
        return "index";
    }

    @GetMapping("/recipe-add")
    public String signUp(Model map, Recipe recipe) {
        map.addAttribute("pageName", "Add New Recipe");

        return "recipe-add";
    }

    @GetMapping("/delete/{id}")
    public String deleteById(@PathVariable("id") Long id, Model model) {
        recipeService.deleteById(id);
        return index(model);
    }

    @GetMapping("/edit/{id}")
    public String editById(@PathVariable("id") Long id, Model model) {
        model.addAttribute("pageName", "Edit New Recipe");

        Recipe recipe = recipeService.getById(id);
        model.addAttribute("recipe", recipe);

        return "recipe-edit";
    }

    @PostMapping
    public String register(@Valid Recipe recipe, BindingResult result, Model model) {
        if (result.hasErrors()) {
            return "recipe-add";
        }

        recipeService.register(recipe);

        return index(model);
    }

    @PostMapping("/update/{id}")
    public String updateRecipe(@PathVariable("id") Long id, @Valid Recipe recipe, BindingResult result, Model model) {
        if (result.hasErrors()) {
            return "recipe-edit";
        }

        recipeService.update(id, recipe);

        return index(model);
    }
}



/*package com.shegoestech.foodie.controller;

import com.shegoestech.foodie.models.CreateValidation;
import com.shegoestech.foodie.models.Recipe;
import com.shegoestech.foodie.models.UpdateValidation;
import com.shegoestech.foodie.service.RecipeRecordService;
import lombok.RequiredArgsConstructor;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.*;

import java.util.Collection;

import static org.springframework.http.MediaType.APPLICATION_JSON_VALUE;

@RequiredArgsConstructor
@RestController
@RequestMapping(value = "/recipe", produces = APPLICATION_JSON_VALUE)
public class RecipeController {

    private final RecipeRecordService recipeRecordService;

    @PostMapping(consumes = APPLICATION_JSON_VALUE)
    public Recipe create(@Validated(CreateValidation.class) @RequestBody Recipe recipe) {
        return recipeRecordService.register(recipe);
    }

    @GetMapping
    public Collection<Recipe> getAll() {
        return recipeRecordService.getAll();
    }

    @GetMapping("/{id}")
    public Recipe getById(@PathVariable("id") Long id) {
        return recipeRecordService.getById(id);
    }

    @PutMapping(value = "/{id}", consumes = APPLICATION_JSON_VALUE)
    public Recipe updateRecipe(@PathVariable("id") Long id,
                                 @Validated(UpdateValidation.class) @RequestBody Recipe recipe) {
        return recipeRecordService.updateRecipe(id, recipe);
    }

    //?vai šo vajag
    @DeleteMapping(value = "/{id}")
    public String updateRecipe(@PathVariable("id") Long id) {
        recipeRecordService.delete(id);
        return "Recipe deleted successfully";
    }

}*/
